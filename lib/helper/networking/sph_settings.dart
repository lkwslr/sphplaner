...
class SPHSettings {
  static Future<bool> changePassword(
    ...
    if (response.statusCode == 200) {
      ...
      if (ikey != "") {
        User? user = await isar.users.getByUsername(StorageProvider.loggedIn);
  ...
  static Future<bool> deleteEMail() async {
    ...
    if (response.statusCode == 200 && response.body == "1") {
      User? user = await StorageProvider.isar.users
          .getByUsername(StorageProvider.loggedIn);
      if (user != null) {
        user.email = "Keine E-Mail-Adresse angegeben";
        return true;
      }
    }
  ...
  static Future<bool> changeEMail(String email) async {
    ...
    if (response.statusCode == 200) {
      dom.Document mailPage = parse(response.body);
      ...
      response = await SPH.post("/benutzerverwaltung.php?a=userMail", data);
      if (response.statusCode == 200 &&
          response.body.contains(
              "An Ihre E-Mail-Adresse wurde ein Bestätigungscode versandt.")) {
        StorageProvider.emailChange = email;
        return true;
      }
    }
    return false;
    ...
    if (StorageProvider.emailChange.isNotEmpty) {
      ...
      await SPH.post("/benutzerverwaltung.php", {
        ...
        'pw': SPH.encrypt(
            await StorageProvider.getPassword(StorageProvider.loggedIn)),
        'neubest': 'ja',
        'mail': StorageProvider.emailChange
      });
    }
  }
      ...
      if (response.statusCode == 200) {
        ...
        if (mailinfo != null) {
          ...
          if (classes.contains("label-success")) {
            ...
            if (user != null) {
              user.email = email;
              StorageProvider.emailChange = "";
              return;
            }
          } else if (classes.contains("label-warning")) {
            String verifyPending =
                mailinfo.getElementsByTagName("span")[0].text;
            String pendingTime = verifyPending
                .split(
                    "E-Mail-Adresse noch nicht bestätigt. Sie haben noch ")[1]
                .split(" ")[0];
            StorageProvider.emailCheck =
                "Neue E-Mail-Adresse innnerhalb von $pendingTime Minuten bestätigen!";
            return;
          } else if (classes.contains("label-danger")) {
            StorageProvider.emailCheck =
                "E-Mail-Verifikationslink abgelaufen. Du kannst diesen in den Einstellungen erneut anfordern.";
            return;
          }
        }
        StorageProvider.emailCheck =
            "E-Mail-Verifikationsprüfung fehlgeschlagen!";
        return;
      }
    }
    StorageProvider.emailCheck = "";
